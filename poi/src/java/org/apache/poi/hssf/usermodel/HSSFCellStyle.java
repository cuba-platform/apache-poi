...
public final class HSSFCellStyle implements CellStyle {
    ...
    public void cloneStyleRelationsFrom(HSSFCellStyle source) {
    	// First we need to clone the extended format
    	//  record
    	_format.cloneStyleFrom(source._format);
    	...
    	if(_workbook != source._workbook) {
			// Then we need to clone the format string,
			//  and update the format record for this    		
    		short fmt = (short)_workbook.getFormat(source.getDataFormatString(), true);
    		setDataFormat(fmt);
    	}
    }
    ...
    public boolean formatEquals(Object obj) {
	...
	public int formatHashCode() {
	    return _format == null ? 0 : _format.hashCode();
	}
